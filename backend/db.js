    ...
    request.post(
        ...
        async function (error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                return getXanderData(answers, job_id);
                // return body["job_id"];
            } else {
                return null;
            }
...
function addAnswersToDb(user_answers, scores) {
    ...
    for (let i = 0; i < user_answers.length; i++) {
        ...
        answer = {
            ...
            score: calculateAnswerScore(answer_scores["workplace_themes"]["workplace_theme_scores"], user_answer.weights), // calc this calculateScore(scores, weights)
...
function calculateUserScore(answers) {
    ...
    db.collection("user").doc(String(user_answers[0].user_id)).update({score: sum});
}

function calculateAnswerScore(scores, weights) {
    let sum = 0;
    scores.filter(score => weights.hasOwnProperty(score["label"])).map(score => sum += score["score"] * weights[score["label"]]);
    return sum * 10;
}
...
module.exports = {getAllUsers, addAnswers}